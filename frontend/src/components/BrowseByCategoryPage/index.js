...
const BrowseByCategoryPage = () => {
    ...
    const categoryStacks = useSelector(state => state.stack.stacks.filter(stack => stack.categoryId === id));
            ...
            {categories && (
                <div className='browse-category-control'>
                    {categories.map((category, i) => {
                        if (category.id === id) return <h2 key={`category-${i}`}>Stacks about {category.name}...</h2>
                    })}
                </div>
            )}
            <div className='browse-category-result'>
                {categoryStacksRendered}
            </div>
        </div>
    )
};
