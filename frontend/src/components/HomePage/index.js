  ...
  const newestStacks = newStacks.map((stack, i) => {
    return (
      <div key={`category-${stack.id}`} className="newest-stack">
        <NavLink className="stack" to={`/stack/${stack.id}`}>
          {stack.name}
        </NavLink>
      </div>
    );
  ...
  return (
    <div className="home-body">
      <div className="welcome">
        ...
        <br />
        <p>
          The beginning of our name "Encepha" derives from the Greek word
          "Encephalon" meaning "brain". Here at EncephaScape we believe in the
          wonders that our brains can perform through studying. Join today to
          access tens of our members stacks and to create your own 'Brainfolio!'
        </p>
      </div>
      <h4>browse our members newest stacks...</h4>
      <div className="recent-stacks">{newestStacks}</div>
